"""
2b_generate_fade_story.py — ФОРК 2_generate_story.py специально под режим
постепенного замещения (macaronic fade, см. CHANGELOG). Сознательно
отдельный файл, не флаг в основном скрипте — 2_generate_story.py остаётся
рабочей лошадкой для обычных книг и этим форком вообще не затрагивается.

Отличие от оригинала: список целевых слов/фраз для фейда — ОБЯЗАТЕЛЬНЫЙ
вход (--target-words), а не опция, и инжектится в промпт каждой главы с
явным требованием повторов и запретом дробить устойчивые словосочетания
(см. TARGET_BLOCK_TEMPLATE и формат файла ниже).

Генерирует оригинальный (без авторских прав) рассказ на иврите под
заданный уровень через Gemini API, затем отдельным вызовом того же API
расставляет огласовки (никуд).

На выходе — .txt файл, готовый как вход для 5_gemini_pipeline.py (тот
режет текст на предложения и размечает перевод/грамматику/корни) и ЗАТЕМ
для 7_fade_stager.py (тот же файл --target-words идёт туда же), и сайдкар
.meta.json с кратким описанием сюжета и ключевыми словами для будущих
иллюстраций/мнемоники.

Формат --target-words (тот же файл, что и для 7_fade_stager.py):
    [
      {"lemma": "רָחוֹק", "tr": "далеко"},
      {"phrase": ["גַּן", "חַיָּה"], "tr": "зоопарк"}
    ]
"lemma" — самостоятельное слово. "phrase" — фиксированное словосочетание
(смихут/идиома) в порядке лемм как в тексте — используется целиком, не
дробится по словам (решает проблему разорванного смихута на этапе
курации списка, а не эвристикой постфактум).
...
Запуск:
    ...
    python3 2b_generate_fade_story.py bet "премиза" --out story.txt \
        --target-words targets.json --beats beats.txt
"""
...
СЛОВА И СЛОВОСОЧЕТАНИЯ ДЛЯ ЦЕЛЕНАПРАВЛЕННОГО ПОВТОРЕНИЯ (режим
постепенного замещения — это ОСНОВНАЯ задача книги наравне с сюжетом, не
второстепенное пожелание):

Самостоятельные слова (используй каждое как обычное слово в предложении,
минимум 4 раза за всю книгу, желательно в разных главах, с разрывом
между появлениями — не скучивай все повторы в одном месте):
{lemma_list}

Устойчивые словосочетания (используй КАЖДЫЙ раз ЦЕЛИКОМ, в точности
этими словами подряд — это готовая единица, которую нужно выучить как
одно целое, не заменяй синонимами и не разбивай другими словами):
{phrase_list}

Общее правило: не заменяй эти слова/сочетания синонимами и не пропускай
их — важна именно частота повторения ОДНИХ и тех же единиц, чтобы
читатель их выучил через повтор.
"""
...
def load_targets(path):
    ...
    lemmas = [item for item in data if "lemma" in item]
    phrases = [item for item in data if "phrase" in item]
    return lemmas, phrases


def format_target_block(lemmas, phrases):
    lemma_list = "\n".join(f"- {w['lemma']} ({w['tr']})" for w in lemmas) or "(нет)"
    phrase_list = "\n".join(f"- {' '.join(p['phrase'])} ({p['tr']})" for p in phrases) or "(нет)"
    return TARGET_BLOCK_TEMPLATE.format(lemma_list=lemma_list, phrase_list=phrase_list)


def generate_chapter(client, model, level, premise, chapter_num, total_chapters,
                      chapter_words, total_words, target_block, summary_so_far=None,
                      prev_text=None, beat=None):
...
def generate_story_chunked(client, model, level, premise, total_words, chapter_words,
                            out_path, resume, target_block, max_chapters=30, beats=None):
    ...
    while len(chapters) < total_chapters:
        ...
        try:
            chapter = call_with_retry(
                ...
                chapter_num, total_chapters, chapter_words, total_words, target_block,
...
def generate_story(client, model, level, premise, word_count, target_block):
    profile = LEVEL_PROFILES[level]
...
def main():
    ...
    ap.add_argument("--target-words", required=True,
                     help="ОБЯЗАТЕЛЬНО для этого скрипта: JSON-файл со словами/фразами для "
                          "фейда (формат см. в докстринге вверху файла). Тот же файл идёт "
                          "потом в 7_fade_stager.py.")
    args = ap.parse_args()
    ...
    lemmas, phrases = load_targets(args.target_words)
    target_block = format_target_block(lemmas, phrases)
    print(f"Целевые слова: {len(lemmas)}, целевые словосочетания: {len(phrases)}.", file=sys.stderr)
    ...
    if chunked:
        ...
        story, progress_path = generate_story_chunked(
            ...
            effective_chapter_words, out_path, args.resume, target_block, args.max_chapters, beats,
        )
        ...
        if args.resume and raw_path.exists():
            ...
            story = generate_story(client, args.model, args.level, args.premise, word_count, target_block)
