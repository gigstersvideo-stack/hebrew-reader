...
Запуск:
    python 8_audio_qa.py book-data.json audio_dir [--model medium] [--threshold 0.35]

Живой прогон по всей библиотеке (2026-09-10) моделью "small" дал 16
подозрительных предложений из ~2500 — при повторной проверке той же
16-строчной выборки моделью "medium" коэффициент несовпадения упал с
0.36–0.62 до 0.00–0.24 на КАЖДОЙ из них, текст практически совпал с
ожидаемым. Ни одного случая с почерком реального бага (конкретное слово
стабильно и правдоподобно читается ДРУГИМ словом, как קוֹף->kevof) — то
есть на "small" это был почти чистый шум модели на коротких предложениях
(3-5 слов), а не реальные проблемы TTS. Поэтому модель по умолчанию —
"medium", несмотря на то что она заметно медленнее на CPU: для
эпизодической QA-проверки (не часть горячего пути генерации) точность
важнее скорости — "small" даёт слишком много ложных срабатываний, "base"
ещё хуже. Если после этого всё равно есть сомнения по короткому
предложению — можно дополнительно прогнать конкретный клип через whisper
напрямую и послушать вживую.
"""
